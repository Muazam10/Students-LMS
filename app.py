...
def save_students(students: List[Dict]) -> None:
    """
    Saves the list of students to JSON file.
    """
...
def add_student(name: str, roll_no: str, marks: float) -> None:
    """
    Adds a new student record after checking roll number uniqueness.
    """
...
def upload_file(file) -> None:
    """
    Handles CSV/XLSX file upload and adds student records.
    """
    try:
    ...
    st.success(f"📥 File processed. Added: {added}, Skipped: {skipped}")

def download_csv() -> bytes:
    """
    Converts student records to downloadable CSV bytes.
    """
    students = load_students()
    if not students:
        return b""
...
def show_statistics():
    """
    Displays total students, average marks and grade distribution.
    """
    students = load_students()
    if not students:
...
if csv_data:
    st.download_button("Download CSV", data=csv_data, file_name="students_data.csv", mime="text/csv")
else:
    st.info("No data available to download.")

# ---------- Sidebar (Simulated Toggle) ----------
with st.expander("☰ Click to Open Sidebar Menu", expanded=False):
    st.sidebar.header("📂 Menu")
    sidebar_option = st.sidebar.radio("Choose Option", [
        "None",
        "Upload File",
        "View All Students",
        "Update Marks",
